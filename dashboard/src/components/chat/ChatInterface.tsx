...
export function ChatInterface({ history, onSend, loading }: { history: ChatMessage[], onSend: (q: string) => void, loading: boolean }) {
  ...
  return (
    <motion.div 
      ...
      className="fixed bottom-0 left-0 right-0 bg-zinc-950/90 backdrop-blur-2xl border-t border-zinc-800/50 z-50 shadow-2xl transition-all duration-300"
      ...
      <div className="max-w-5xl mx-auto h-full flex flex-col">
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`h-1 w-1 rounded-full ${loading ? 'bg-primary animate-pulse' : 'bg-zinc-700'}`} />
            <span className="text-[10px] font-black tracking-[0.3em] text-zinc-500 uppercase group-hover:text-primary transition-colors">
              {loading ? "Syncing Neural Network..." : "Serqet Terminal"}
            </span>
          </div>
          ...
          <AnimatePresence>
            {isExpanded && history.map((msg, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-4 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-zinc-800 border border-zinc-700' : 'bg-primary/5 border border-primary/20'}`}>
        ...
        <div className="p-4 bg-transparent">
          <form onSubmit={handleSubmit} className="flex gap-3">
            <div className="relative flex-1 group">
              <Terminal className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-primary transition-colors" size={14} />
              <Input 
                value={input} 
            ...
            <Button type="submit" disabled={loading} className="h-12 px-8 bg-primary hover:opacity-80 text-white font-black rounded-xl border-t border-white/10">
              RUN
            </Button>
          </form>
        </div>
